...
\begin{question}\label{qst: Unabhängigkeit von Umgebung}
...
Die Wohldefiniertheit, also Unabhängigkeit von $V$, folgt aus Übung \ref{qst: Unabhängigkeit von Umgebung}.


% Verträglichkeit der Notationen
...
\begin{enumerate}
 \item
  Für $y \in \R^m$ ist genau dann $\lim_{x \to \infty} f(x) = y$, wenn $\lim_{x \downarrow} f(1/x) = y$. Die Aussage gilt auch für $y \in \{-\infty, \infty\}$.
 \item
