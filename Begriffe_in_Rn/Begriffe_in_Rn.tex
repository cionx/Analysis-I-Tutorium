...
Wir kennen den Satz von Bolzano-Weierstraß, der besagt, dass jede beschränkt Folge auf $\R$ eine konvergente Teilfolge besitzt. Die naheliegende Verallgemeinerung gilt auch für $\R^n$.
...
\end{prop}
\begin{proof}
 In Koordinaten sei $x_n = (x^{(1)}_n, \dotsc, x^{(m)}_n)$ für alle $n \in \N$. Da die Folge $(x_n)$ beschränkt ist, ist die Folge $(x^{(i)}_n)_{n \in \N}$ für alle $1 \leq i \leq m$ beschränkt.
 ...
 \]
 Da $x^{(1)}_{n_{1,j}} \to x^{(1)}$ für $j \to \infty$ ist auch $x^{(1)}_{n_{2,j}} \to x^{(1)}$ für $j \to \infty$.
...
\begin{question}
 Es sei $f \colon \R \to \R$ eine stetige Funktion. Zeigen Sie, dass der Grahp
 \[
